...
using Container = GlazeWM.Domain.Containers.Container;
...
internal sealed class ManageWindowHandler : ICommandHandler<ManageWindowCommand>
{
  ...
  public CommandResponse Handle(ManageWindowCommand command)
  {
    ...
    (var targetParent, var targetIndex) = command.TargetParent != null ? (command.TargetParent, 0) : GetInsertionTarget();
  ...
  private (SplitContainer targetParent, int targetIndex) GetInsertionTarget()
  {
    ...
    if (focusedContainer is Workspace workspace)
    {
      return (workspace, 0);
    }

    return ((SplitContainer)focusedContainer.Parent, focusedContainer.Index + 1);
  }
}
