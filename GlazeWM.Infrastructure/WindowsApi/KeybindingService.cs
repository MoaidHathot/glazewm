...
public class KeybindingService
{
  ...
  public void AddGlobalKeybinding(string keybindingString, Action callback)
  {
    ...
    if (_keybindingsByTriggerKey.TryGetValue(triggerKey, out var value))
    {
      value.Add(keybinding);
      return;
    }

    _keybindingsByTriggerKey.Add(triggerKey, [keybinding]);
  }
...
#pragma warning disable CS8605 // Unboxing a possibly null value.
    var inputEvent = (LowLevelKeyboardInputEvent)Marshal.PtrToStructure(lParam, typeof(LowLevelKeyboardInputEvent));
#pragma warning restore CS8605 // Unboxing a possibly null value.
    ...
    var matchedKeybindings = registeredKeybindings.Where(keybinding =>
    {
      return keybinding.KeyCombination.All(key =>
      {
        ...
        if (cachedKeyStates.TryGetValue(key, out var value))
        {
          return value;
        }
    ...
    var hasModifierKeysToReject = modifierKeysToReject.Any((modifierKey) =>
      cachedKeyStates.TryGetValue(modifierKey, out var value) ? value : IsKeyDown(modifierKey)
    );
